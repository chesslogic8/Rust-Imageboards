...
pub const RESET_DB_ON_START: bool = true; // <--- set to true to drop & recreate tables on every run
// ===== End Board Settings =====
...
pub static DB: Lazy<Mutex<PooledConn>> = Lazy::new(|| {
    let pool = Pool::new("mysql://devuser:devpass@localhost:3306/chessboard").expect("Failed to connect to MariaDB/MySQL");
...
pub fn get_threads_paged(threads_per_page: usize, page: usize) -> Vec<Thread> {
    let mut db = DB.lock().unwrap();
    db.exec_map(
        "SELECT id, subject, message, media, bumped FROM threads ORDER BY bumped DESC LIMIT :limit OFFSET :offset",
        params! { "limit" => threads_per_page as u64, "offset" => (threads_per_page * page) as u64 },
        |(id, subject, message, media, bumped): (i64, String, String, Option<String>, i64)| {
            let preview = if message.len() > THREAD_PREVIEW_LENGTH {
                format!("{}...", &message[..THREAD_PREVIEW_LENGTH])
            } else {
                message.clone()
            };
            Thread { id, subject, message, media, preview, bumped }
        }
    ).unwrap_or_default()
}

pub fn get_total_thread_count() -> usize {
    let mut db = DB.lock().unwrap();
    db.query_first::<u64, _>("SELECT COUNT(*) FROM threads")
...
pub fn get_thread(id: i64) -> Option<(Thread, Vec<Post>)> {
    ...
    let thread = db.exec_first(
        "SELECT id, subject, message, media, bumped FROM threads WHERE id = :id",
        params! { "id" => id }
    ).unwrap_or(None)
     .map(|(id, subject, message, media, bumped): (i64, String, String, Option<String>, i64)| {
        let preview = if message.len() > THREAD_PREVIEW_LENGTH {
            format!("{}...", &message[..THREAD_PREVIEW_LENGTH])
        } else {
            message.clone()
...
pub fn insert_thread(subject: &str, message: &str, media: Option<&str>) {
    let mut db = DB.lock().unwrap();
    let now = chrono::Utc::now().timestamp();
    db.exec_drop(
        "INSERT INTO threads (subject, message, media, bumped) VALUES (:subject, :message, :media, :bumped)",
        params! { "subject" => subject, "message" => message, "media" => media, "bumped" => now },
    ).unwrap();
}
